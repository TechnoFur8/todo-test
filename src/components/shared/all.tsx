...
export const All = ({ setActiveTab, activeTab }: Props) => {
        ...
        <Accordion collapsible type="single" defaultValue="1">
            <AccordionItem value="1">
                ...
                <div className={"flex items-center justify-between bg-white shadow py-2 border-t-zinc-300 border-1 px-3"}>
                    <span className={"text-zinc-400 font-normal"}>{todos.length}</span>
                    <div className={"space-x-2"}>
                        <Button variant={"link"} className={cn("cursor-pointer text-zinc-400 font-normal", activeTab === "ALL" && "underline")} onClick={() => setActiveTab("ALL")}>All</Button>
                        <Button variant={"link"} className={"cursor-pointer text-zinc-400 font-normal"} onClick={() => setActiveTab("ACTIVE")}>Active</Button>
